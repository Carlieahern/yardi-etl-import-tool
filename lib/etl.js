...
function dataRowsForChunk(chunk, mapByRp, config) {
  ...
  for (const prop of chunk) {
    ...
    for (const row of prop.exportRows) {
      ...
      const monthly = row.needsSignSwitch
        ? row.values.map((v) => (v === 0 ? 0 : -v))
        : row.values;

      dataRows.push([tranNum, bookNum, budgetYear, yardiId, row.gl, ...monthly]);
      tranNum += 1;
    }
  }
  return dataRows;
...
// Generate one or more Yardi ETL CSV files, splitting into batches of at most
// `propertiesPerFile` properties each. Returns [{ csv, propertyCount, rowCount }].
export function generateCsvFiles(
  ...
  return chunks.map((chunk) => {
    const dataRows = dataRowsForChunk(chunk, mapByRp, config);
    const aoa = [["FinBudgets"], ETL_HEADER, ...dataRows];
    return {
      csv: Papa.unparse(aoa, { newline: "\r\n" }),
